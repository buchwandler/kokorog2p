...
- `language_routing={"mode": "auto", "languages": ["de", "en"]}` enables optional
  ...
  language, including `pt-pt`, may be listed; the allowlist is never expanded
  implicitly.
- `LanguageRoutingConfig` is also accepted.
- `g2p_resolver(language)` supplies and caches a configured frontend per canonical
...
in the default language. Candidate failures are isolated and warned. Explicit `ph`,
`phonemes`, `lang`, and `language` spans take precedence.
...
`LanguageRoute` objects in `PhonemizeResult.language_routes`, with selected lexicon
provenance. Do not pass arbitrary written text when number, date, unit, currency, or
abbreviation expansion is expected.
