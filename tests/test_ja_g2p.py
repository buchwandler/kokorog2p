...
class TestJapaneseFrontendMapping:
    ...
    def test_internal_frontend_apostrophe_is_ignored_when_mora_count_matches(
        self,
    ) -> None:
        frontend = frontend_for(record("キツネ", "キ’ツネ", 3))
        g2p = JapaneseG2P(frontend=frontend)
        assert g2p.phonemize("キツネ")
    ...
    def test_internal_frontend_apostrophe_requires_matching_mora_count(self) -> None:
        frontend = frontend_for(record("キツネ", "キ’ツネ", 4))
        g2p = JapaneseG2P(frontend=frontend)
