...
def test_portuguese_factory_routes_dialect_without_semantic_preparation(
...
def test_arabic_factory_aliases_and_native_profile() -> None:
    ...
    instances = [
        get_g2p(alias, diacritizer="none", lexicons=())
...
def test_arabic_espeak_backend_remains_generic() -> None:
    ...
    assert isinstance(get_g2p("ar", backend="espeak", lexicons=()), EspeakOnlyG2P)
...
def test_kazakh_factory_aliases_share_cached_native_instance() -> None:
    ...
    clear_cache()
    instances = [
        get_g2p(alias, lexicons=()) for alias in ("kk", "kk-kz", "kaz", "kazakh")
    ]
