...
class ItalianNormalizer(TextNormalizer):
    """Prepare Italian semantics upstream and retain kokorog2p typography."""
    ...
    def __init__(
        ...
        """Initialize the Italian downstream adapter."""
    ...
    def _initialize_rules(self) -> None:
        """Initialize Italian typography rules only."""
        self.add_rule(
            NormalizationRule(
    ...
    def normalize(
        ...
        *,
        ...
        """Prepare Italian semantics with spokenform, then apply typography."""
        if not text:
            return text, []
        ...
        config = replace(
            PreparationConfig.for_kokorog2p("it"),
    ...
    def __call__(
        ...
        *,
        ...
        """Normalize Italian text without tracking change details."""
    ...
    def iter_structured_replacements(
        ...
        *,
        ...
        """Return spokenform source-aligned replacements for Italian forms."""
        return iter(
            TextReplacement(
                start=item.start,
                end=item.end,
    ...
    def normalize_for_g2p(self, text: str) -> str:
        """Apply only Italian typography after semantic preparation."""
    ...
    def normalize_token(
        ...
        *,
        ...
        """Normalize token typography without re-running Italian semantics."""
        if not text:
            return text
